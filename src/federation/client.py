...
class Client:
    def __init__(self, context):
        ...
        if my_client is None:
            raise ValueError(
                f"Client name {context.args.client_name} not found in configuration file {context.args.fed_config}"
            )
        ...
        if self.client_config.data_type == "txt":
            ...
            raise ValueError(
                f"client_config.data_type={self.client_config.data_type} is not implemented by federation.client.Client"
            )
    ...
    def setup_data_loader(self):
        ...
        self.global_eval_dataloader = global_dataloader[
        ...
        if data_type == "txt":
            ...
            train_loaders, test_set = get_FL_trainloader(
                dataset,
            ...
            self.trainer = ClientTrainer(
                ...
                inter_distance=4,
                client_id=data_partition_index,
                wandb=self.wandb,
            )
            ...
            config.model.cache_dir = (
                config.model.cache_dir + "-" + config.train.server_dataset
            )
            config.train.output_file = os.path.join(
                config.model.cache_dir, config.train.output_file
            )
            config.train.best_model_save_path = os.path.join(
                config.model.cache_dir, config.train.best_model_save_path
            )
            config.train.model_save_path = os.path.join(
                config.model.cache_dir, config.train.model_save_path
            )
            config.model.embed_dim = self.args.feature_dim
            config.model.not_bert = True

            self.trainer = MMClientTrainer(
                args,
                config,
                self.logger,
                client=data_partition_index,
                dset_name="flicker30k",
                device="cuda",
            ...
            raise ValueError(
                f"client_config.data_type={data_type} is not implemented by federation.client.get_data_loader()"
            )
    ...
    def run(self):
        ...
        while True:
            ...
            server_state = api.get_server_state(
                self.context, expected_state=api.RoundState.COLLECT
            )
            self.logger.log(
                f"Server state: {server_state.round_state}, round number: {server_state.round_number}, global feature hash: {server_state.feature_hash}"
            )
            ...
            del global_feature
            api.add_local_repr(
                self.context, server_state, img, txt, -1
            )  # todo: set local rounds
            del img, txt
