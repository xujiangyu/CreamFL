...
def evaluate_single(output, f_ids):
    ...
    retrieved_items, retrieved_scores, _ = engine.evaluator.retrieve(
        image_features,
        caption_features,
        q_id,
        torch.tensor(f_ids),
...
def set_global():
    ...
    if current_state.round_state != api.RoundState.BUSY:
        return (
            json.dumps(
                {
                    "status": "error",
                    "message": "The server should not update the global model at this time.",
                }
            ),
            HTTPStatus.CONFLICT,
        )
    ...
    if round_number != str(current_state.round_number):
        return (
            json.dumps(
                {
                    "status": "error",
                    "message": f"The round has passed. expected={current_state.round_number}, got={round_number}",
                }
            ),
            HTTPStatus.CONFLICT,
        )
    ...
    if old_feature_hash != current_state.feature_hash:
        return (
            json.dumps(
                {
                    "status": "error",
                    "message": f"The global model has changed {old_feature_hash}!={current_state.feature_hash}",
                }
            ),
            HTTPStatus.CONFLICT,
        )
    ...
    if new_feature_hash == "":
        return (
            json.dumps(
                {"status": "error", "message": "The new feature hash is required"}
            ),
...
def add_client():
    ...
    if current_state.round_state == api.RoundState.BUSY:
        return (
            json.dumps(
                {
                    "status": "error",
                    "message": "The server is not accepting clients at this time.",
                }
            ),
            HTTPStatus.CONFLICT,
        )
    ...
    if feature_hash != current_state.feature_hash:
        return (
            json.dumps({"status": "error", "message": "The global model has changed"}),
            HTTPStatus.CONFLICT,
        )
    ...
    if (
        len(current_state.clients_reported)
        == server_context.fed_config.server["max_clients"]
    ):
        current_state.advance_round()
        server_context.logger.log(
            f"Global round {current_state.round_number} has collected the max number of clients."
        )
