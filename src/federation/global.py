...
from src.federation import api
...
class Global:
    ...
    def load_dataset(self):
        ...
        self.evaluator = COCOEvaluator(
            eval_method="matmul", verbose=True, eval_device="cuda", n_crossfolds=5
        )
        self.engine.create(
            self.config, self.vocab.word2idx, self.evaluator, args.mlp_local
        )

        self.train_eval_dataloader = (
            self._dataloaders.pop("train_subset_eval" + f"_{args.pub_data_num}")
    ...
    def train(self, server_state: api.ServerState):
        ...
        if (
            self.args.agg_method == "con_w"
            or self.args.contrast_local_intra
            or self.args.contrast_local_inter
        ):
        ...
        global_feature = api.GlobalFeature(
            self.global_img_feature, self.global_txt_feature, self.distill_index
        )
        ...
        new_server_state = api.get_server_state(
            self.context, expected_state=api.RoundState.BUSY
        )
        if new_server_state.round_number != server_state.round_number + 1:
            self.logger.log(
                f"global train failed: round number mismatch: {new_server_state.round_number} != {server_state.round_number}"
            )
            return new_server_state, False
        if new_server_state.feature_hash != global_feature.hash:
            self.logger.log(
                f"global train failed: feature hash mismatch: {new_server_state.feature_hash} != {global_feature.hash}"
            )
            return new_server_state, False
        img_vec, txt_vec = api.get_clients_repr(
            self.context, new_server_state.clients_reported
        )
        self.logger.log(
            f"loaded client representations: img_vec x{len(img_vec)}, txt_vec x{len(txt_vec)}"
        )

        # global distillation
        if not self.args.disable_distill:
            self.distill(
                new_server_state.round_number, img_vec, txt_vec, self.distill_index
            )
        ...
        rsum10 = (
            test_scores["test"]["n_fold"]["i2t"]["recall_10"]
            + test_scores["test"]["n_fold"]["t2i"]["recall_10"]
            + test_scores["test"]["i2t"]["recall_10"]
            + test_scores["test"]["t2i"]["recall_10"]
        )

        self.wandb.log({"Server rsum_r1": rsum}, step=round_n)
        self.wandb.log(
            {"Server n_fold_i2t_r1": test_scores["test"]["n_fold"]["i2t"]["recall_1"]},
            step=round_n,
        )
        self.wandb.log(
            {"Server n_fold_t2i_r1": test_scores["test"]["n_fold"]["t2i"]["recall_1"]},
            step=round_n,
        )
        self.wandb.log(
            {"Server i2t_r1": test_scores["test"]["i2t"]["recall_1"]}, step=round_n
        )
        self.wandb.log(
            {"Server t2i_r1": test_scores["test"]["t2i"]["recall_1"]}, step=round_n
        )
    ...
    def distill(self, round_n, img_vec, txt_vec, distill_index):
        ...
        if (
            self.config.model.use_img_client
            or self.config.model.use_txt_client
            or self.config.model.use_mm_client
        ):
            ...
            if self.args.agg_method == "con_w":
                if not i_vec:
                    ...
                    if not contrastive_w:
                        self.logger.log(
                            "distill.aggregation No tensors were added to contrastive_w for images"
                        )
                    else:
                        contrastive_w = torch.softmax(
                            torch.cat(contrastive_w, dim=0), dim=0
                        )
                        for i in range(len(i_vec)):
                            i_vec[i] = (
                                i_vec[i] * contrastive_w[i].reshape(-1, 1)
                            ).unsqueeze(0)
                    i_vec = torch.sum(
                ...
                if not t_vec:
                    ...
                    if not contrastive_w:
                        self.logger.log(
                            "distill.aggregation No tensors were added to contrastive_w for texts"
                        )
                    else:
                        contrastive_w = torch.softmax(
                            torch.cat(contrastive_w, dim=0), dim=0
                        ...
                        for i in range(len(t_vec)):
                            t_vec[i] = (
                                t_vec[i] * contrastive_w[i].reshape(-1, 1)
                            ).unsqueeze(0)
                    t_vec = torch.sum(
                        torch.cat(t_vec, dim=0), dim=0
                    )  # aggregated text vectors
            else:
        ...
        for idx, (images, captions, captions_word, caption_lens, _, _, index) in tqdm(
            enumerate(
                self.dataloaders_global["train_subset" + f"_{self.args.pub_data_num}"]
            )
        ):
            images = images.to(self.engine.device)  # [bs, 3, 224, 224]
            captions = captions.to(self.engine.device)  # [bs, seq_len]
            ...
            if self.config.train.grad_clip > 0:
                nn.utils.clip_grad.clip_grad_norm_(
                    self.engine.model.parameters(), self.config.train.grad_clip
                )
...
if __name__ == "__main__":
    ...
    global_compute.engine.report_scores(
        step=context.args.comm_rounds,
