...
from src.federation.context import Context
...
class ServerState:
    def __init__(
        ...
        clients_reported: Optional[
            Dict[str, ClientState]
        ] = None,  # map of names to ClientState that have reported their updates
        feature_hash: str = "",
    ):
    ...
    def to_dict(self):
        return {
            ...
            "clients_reported": {
                k: v.to_dict() for k, v in self.clients_reported.items()
            },
    ...
    def from_dict(cls, data):
        return cls(
            ...
            clients_reported={
                k: ClientState.from_dict(v) for k, v in data["clients_reported"].items()
            },
...
def get_global_dataloader(context: Context):
    ...
    return prepare_coco_dataloaders(
        context.config.dataloader,
        dataset_root,
        context.args.pub_data_num,
        context.args.max_size,
        vocab_path,
    )
...
def add_local_repr(
    context: Context, expected_server_state: ServerState, img, txt, local_rounds: int
):
    """
    ...
    client_state = ClientState(
        name=context.args.client_name,
        img_model=context.has_img_model,
        txt_model=context.has_txt_model,
    ...
    url = (
        get_api_url(context)
        + f"/add_client?round_number={expected_server_state.round_number}&feature_hash={expected_server_state.feature_hash}"
    )
    ...
    while True:
        try:
            ...
            if resp.status_code == 200:
                ...
                return True
            context.logger.log(
                f"Can not add local representations to server. Status code: {resp.status_code} body: {resp.text}"
            )
            return False
        except Exception as e:
            error_sleep(context, e)
...
def submit_global_feature(
    context: Context, state: ServerState, global_feature: GlobalFeature
):
    context.logger.log(f"Saving global feature to file.")
    _, hash = global_feature.save(context.fed_config.feature_store)
    url = (
        get_api_url(context)
        + f"/set_global_feature?round_number={state.round_number}&old_feature_hash={state.feature_hash}&new_feature_hash={hash}"
    )
    ...
    while True:
        try:
            ...
            if resp.status_code == 200:
                ...
                return True
            context.logger.log(
                f"Can not submit global feature to server. Status code: {resp.status_code} body: {resp.text}"
            )
...
def get_clients_repr(context: Context, clients_reported: Dict[str, ClientState]):
    ...
    for name, client in clients_reported.items():
        if client.img_model:
            img = load(
                torch.Tensor, context.fed_config.feature_store, client.img_model_hash
            )
            img_vec.append(img)
        if client.txt_model:
            txt = load(
                torch.Tensor, context.fed_config.feature_store, client.txt_model_hash
            )
            txt_vec.append(txt)
